...
export function DailyCheckIn({ habits, onCheckIn, onHabitUpdated }: DailyCheckInProps) {
  ...
  const [soundType, setSoundType] = useState<"success" | "miss">("success")
  ...
  const handleCheckIn = async (habitId: string, completed: boolean, notes?: string) => {
    ...
    try {
      ...
      console.log(`Daily Check-In: Recording habit "${currentHabit?.name}" as ${completed ? "COMPLETED" : "MISSED"}`, {
        habitId,
        completed,
        notes: notesToSend,
  ...
  return (
    <Card className="shadow-lg border-none bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm">
      {playSound && (
        <audio
          src={soundType === "success" ? "/success-sound.mp3" : "/miss-sound.mp3"}
          autoPlay
